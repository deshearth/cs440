...
class Assign:
    ...
    def simulation(self, var, val):
        ...
        assigned = copy.deecopy(self.assigned)
    ...
    def copy(self):
        return Assign(dict(self.assigned),
                      dict(self.unassigned),
                      np.array(self.grid), self) 
    def deepcopy(self):
...
def solveSudoku(csp):
    ...
    result, nodeNum = backtrack(assignment, csp)
...
def backtrack(assignment, csp, nn=0):
    ...
    for val in orderDomainVal(assignment,csp, var):
        ...
        if csp.constraints(assignment, var, val): # node consistancy
            attempt = assignment.copy()
            attempt.modify(var, val, 'add')
            result, nodeNum = backtrack(attempt, csp, nodeNum)
            if result != failure:
